...
class AgentManager:
    ...
    def load_agents_from_directory(
        self, directory_path: str, reasoning_effort: Optional[float] = None
        ...
        yaml_files.extend(
            glob.glob(os.path.join(directory_path, "**/*.yaml"), recursive=True)
        )
    ...
    def create_agent_from_yaml(
        self, definition_file_path: str, reasoning_effort: Optional[float] = None
    ) -> ChatCompletionAgent:
        """
